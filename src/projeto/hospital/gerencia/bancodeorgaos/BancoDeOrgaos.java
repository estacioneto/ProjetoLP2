...
public class BancoDeOrgaos {
	...
	public void adicionaOrgao(String nome,
			String tipoSanguineo) {
		try {
			ValidadorDeDados.validaString(Constantes.NOME, nome);
			ValidadorDeDados.validaString(Constantes.TIPOS_SANGUINEO,
					tipoSanguineo);

			this.orgaos.add(new Orgao(nome, tipoSanguineo));
		} catch (DadoInvalidoException excecao) {
			throw new OperacaoInvalidaException(
					MensagensDeErro.ERRO_CADASTRO_ORGAO + excecao.getMessage());
		}
	}
	
	public void removeOrgao(String nome,
			String tipoSanguineo) {
		try {
			ValidadorDeDados.validaString(Constantes.NOME, nome);
			ValidadorDeDados.validaString(Constantes.TIPOS_SANGUINEO,
			...
			throw new OperacaoInvalidaException(
					MensagensDeErro.ERRO_REMOCAO_ORGAO + excecao.getMessage());
		}
	}
	
	public Orgao getOrgao(String nome,
			String tipoSanguineo) {
		try {
			ValidadorDeDados.validaString(Constantes.NOME, nome);
			ValidadorDeDados.validaString(Constantes.TIPOS_SANGUINEO,
	...
	public int getQuantidadeOrgao(String nome){
		try{
			...
			throw new OperacaoInvalidaException(
					MensagensDeErro.ERRO_CONSULTA_ORGAO + excecao.getMessage());
		}
	}
